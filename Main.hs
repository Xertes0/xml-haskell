...
data XMLValue = XMLTag (String, [XMLAttribute], XMLValue)
              | XMLString String
              | XMLArray [XMLValue]
              deriving Show
...
exhaust parser = Parser $ \input -> checkNull
                                    $ last
                                    $ takeWhile isJust
                                    $ iterate f (Just (input, []))
  where f prev | Nothing <- prev = Nothing
               | Just (input', output') <- prev = do
                   (input'', output'') <- runParser parser input'
                   return (input'', output' ++ [output''])
        checkNull x | Just (_, []) <- x = Nothing
                    | otherwise = x
...
parseTag = (\name attrs body endName -> XMLTag (ensure name endName, attrs, body))
           <$> (parseChar '<' *> parseFailOnChar '/' *> parseUntil (\c -> (c == '>') || isSpace c))
           <*> ((parseWS *> ((\_ -> []) <$> parseChar '>')) <|> (parseAttributes <* parseChar '>'))
           <*> (parseTags <|> (XMLString <$> parseUntil (== '<')))
           <*> (parseWS *> parseString "</" *> parseUntil (== '>') <* parseChar '>')
  where ensure a b | a == b = a
                   | otherwise = error $ "Tag begins and ends with a different name '"++a++"' - '"++b++"'"

parseRoot :: Parser XMLValue
parseRoot = (\attrs body -> XMLTag ("root", attrs, body))
